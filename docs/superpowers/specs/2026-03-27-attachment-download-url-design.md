...
- `cleanup_expired() -> Vec<PathBuf>` — removes expired entries, returns list of file paths to delete
...
**No security headers middleware** on this route (no CSP/X-Frame-Options needed for file downloads).
...
Pass `Arc<AttachmentStore>` and `issuer` string to `ExchangeMcpServer::new()` so the tool can generate URLs and register tokens.
...
- **Expiration**: tokens and files are cleaned up after 24h, limiting exposure window
...
| `src/lib.rs` or `main.rs`   | `mod attachment_store;`                         |
